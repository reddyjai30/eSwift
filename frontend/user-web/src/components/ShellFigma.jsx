  ...
  return (
    <button
      ...
      style={{
        ...
        gap:4, padding:'8px 16px', minWidth:60, borderRadius:12,
    ...
    >
      <div style={{ width:20, height:20, borderRadius:4, background:'var(--divider)' }} />
      ...
      <header style={{ position:'sticky', top:0, zIndex:40, background:'var(--bg-paper)', borderBottom:'1px solid var(--divider)', boxShadow:'var(--e-1)' }}>
        <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', height:56, padding:'0 16px', maxWidth:1080, margin:'0 auto' }}>
          ...
          <div style={{ display:'flex', alignItems:'center', gap:8 }}>
            <button onClick={()=>navigate('/orders')} style={{ padding:'6px 12px', borderRadius:10, fontSize:14, color:'var(--text-secondary)' }}>Orders</button>
            <button onClick={()=>navigate('/wallet')} style={{ padding:'6px 12px', borderRadius:10, fontSize:14, color:'var(--text-secondary)' }}>Wallet</button>
            <button onClick={()=>navigate('/cart')} style={{ position:'relative', padding:'6px 12px', borderRadius:10, fontSize:14, color:'var(--text-secondary)' }}>
              Cart
              {count>0 && (
